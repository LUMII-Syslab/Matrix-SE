...
class ResidualShuffleExchange2D(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        global saturation_costs
        saturation_costs = []
        global gate_mem
        gate_mem = []
        global reset_mem
        reset_mem = []
        global candidate_mem
        candidate_mem = []
        global prev_mem_list
        prev_mem_list = []
        global residual_list
        residual_list = []
        global info_alpha
        info_alpha = []
    ...
    def call(self, inputs, training=False, **kwargs):
        ...
        last_layer = tf.reshape(inputs, [shape[0], shape[1] * shape[2], shape[3]])

        last_layer, _ = shuffle_exchange_network_heavy_sharing(last_layer, "benes", n_blocks=self.block_count)

        return tf.reshape(last_layer, shape)
